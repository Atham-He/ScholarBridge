...
export default function StudentProfilePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (profileRes.ok) {
        ...
        router.replace(`/login?role=STUDENT&next=${encodeURIComponent('/student/profile')}`);
        return;
      } else {
      ...
      // 获取我的申请
      const applicationsRes = await fetch('/api/student/applications');
  ...
  const toggleSection = (section: 'profile' | 'saved' | 'applications') => {
  ...
  const getApplicationProjectDetail = (app: StudentApplication): ProjectDetail => ({
    ...
    availableSeats:
      typeof app.project.capacity === 'number' && typeof app.project.enrolled === 'number'
        ...
        : undefined,
    mentor: {
      displayName: app.mentor.mentorProfile?.displayName ?? app.mentor.email,
      institution: app.mentor.mentorProfile?.institution ?? '未知',
      department: app.mentor.mentorProfile?.department,
      title: app.mentor.mentorProfile?.title,
      bioShort: app.mentor.mentorProfile?.bioShort,
    },
  });
                    ...
                    <div className="mb-3 flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                      ...
                      {profile.resumeFileName && (
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => window.open('/api/student/profile/resume', '_blank', 'noopener,noreferrer')}
                        >
                          查看简历
                        </Button>
                      )}
                        ...
                        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                          <div>
                            ...
                            <div className="grid gap-1 text-sm text-[#1A1A1A]">
                              <p><span className="font-semibold">导师：</span>{app.mentor.mentorProfile?.displayName ?? app.mentor.email}</p>
                              <p><span className="font-semibold">学校：</span>{app.mentor.mentorProfile?.institution ?? '未知'}</p>
