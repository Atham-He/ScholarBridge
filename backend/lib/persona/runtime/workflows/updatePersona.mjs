...
function buildUpdatedInput({ existingInput = {}, input = {}, copiedUploads = [], publicUrls = [] }) {
  const { copiedWechatUploads = [], copiedMeetingUploads = [], copiedThinkingQuestionnaireUploads = [], ...visibleInput } = input;
  return {
    ...existingInput,
    ...visibleInput,
    ...
    wechatPaths: [
      ...(existingInput.wechatPaths || []),
      ...copiedWechatUploads
    ],
    meetingPaths: [
      ...(existingInput.meetingPaths || []),
      ...copiedMeetingUploads
    ],
    thinkingQuestionnairePaths: [
      ...(existingInput.thinkingQuestionnairePaths || []),
      ...copiedThinkingQuestionnaireUploads
    ],
    mentorSpeaker: input.mentorSpeaker || input.wechatMentorSpeaker || existingInput.mentorSpeaker || '',
    ...
    updates: [
      ...(existingInput.updates || []),
      {
        ...
        wechatCount: copiedWechatUploads.length,
        meetingCount: copiedMeetingUploads.length,
        thinkingQuestionnaireCount: copiedThinkingQuestionnaireUploads.length,
...
export async function updatePersonaWorkflow({ slug, input = {}, config, store, llmProvider, asrProvider }) {
  await store.init();
