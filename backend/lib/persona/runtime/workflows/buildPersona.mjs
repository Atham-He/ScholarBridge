...
export async function copyUploadIntoDir(upload, targetDir) {
  ...
  let transcriptPath = descriptor.transcriptPath;
  if (descriptor.transcriptPath) {
    const transcriptName = sanitizeFileName(path.basename(descriptor.transcriptPath), 'meeting_transcript.txt');
    transcriptPath = path.join(targetDir, `${safeId('transcript')}_${transcriptName}`);
    await fs.copyFile(descriptor.transcriptPath, transcriptPath);
  }
  ...
  return {
    ...
    mentorSpeaker: descriptor.mentorSpeaker,
    meetingSpeaker: descriptor.meetingSpeaker,
    transcriptPath,
...
export async function collectPersonaInputSources({ input, mentor, config, uploadsDir, llmProvider, asrProvider }) {
  const copiedUploads = [];
  const copiedWechatUploads = [];
  const copiedMeetingUploads = [];
  const copiedThinkingQuestionnaireUploads = [];
  ...
  for (const upload of input.wechatPaths || []) {
    const descriptor = normalizeUploadInput(upload);
    const typedUpload = {
      ...descriptor,
      sourceType: 'wechat',
      mimeType: descriptor.mimeType || 'text/plain',
      mentorSpeaker: descriptor.mentorSpeaker || input.mentorSpeaker || input.wechatMentorSpeaker || ''
    };
    try {
      copiedWechatUploads.push(await copyUploadIntoDir(typedUpload, uploadsDir));
    } catch {
      copiedWechatUploads.push(typedUpload);
    }
  }

  for (const upload of input.meetingPaths || []) {
    const descriptor = normalizeUploadInput(upload);
    const typedUpload = {
      ...descriptor,
      sourceType: 'meeting',
      mentorSpeaker: descriptor.mentorSpeaker || descriptor.meetingSpeaker || input.meetingSpeaker || input.mentorSpeaker || ''
    };
    try {
      copiedMeetingUploads.push(await copyUploadIntoDir(typedUpload, uploadsDir));
    } catch {
      copiedMeetingUploads.push(typedUpload);
    }
  }

  for (const upload of input.thinkingQuestionnairePaths || input.questionnairePaths || []) {
    const descriptor = normalizeUploadInput(upload);
    const typedUpload = {
      ...descriptor,
    ...
    try {
      copiedThinkingQuestionnaireUploads.push(await copyUploadIntoDir(typedUpload, uploadsDir));
    } catch {
      copiedThinkingQuestionnaireUploads.push(typedUpload);
    }
  }
  ...
  const uploadSources = await parseUploadFiles([...copiedUploads, ...copiedWechatUploads, ...copiedMeetingUploads, ...copiedThinkingQuestionnaireUploads], llmProvider, {
    mentor,
    mentorSpeaker: input.mentorSpeaker || input.wechatMentorSpeaker || input.meetingSpeaker || '',
    meetingSpeaker: input.meetingSpeaker || '',
    asrProvider,
    tmpDir: config.tmpDir,
    ffmpegPath: config.ffmpegPath
  });
  return { copiedUploads, copiedWechatUploads, copiedMeetingUploads, copiedThinkingQuestionnaireUploads, publicSources, uploadSources };
}

export async function buildPersonaWorkflow({ input, config, store, llmProvider, asrProvider }) {
  await store.init();
  ...
  const { copiedUploads, copiedWechatUploads, copiedMeetingUploads, copiedThinkingQuestionnaireUploads, publicSources, uploadSources } = await collectPersonaInputSources({
    input,
    mentor,
    config,
    uploadsDir,
  ...
  const persona = await distillPersona({ mentor, sources, chunks, llmProvider, config });
  ...
  const bundle = {
    input: {
      ...
      wechatPaths: copiedWechatUploads,
      meetingPaths: copiedMeetingUploads,
      thinkingQuestionnairePaths: copiedThinkingQuestionnaireUploads
    },
    sources,
    chunks,
    persona,
  ...
  return {
    ...
    uploadSourceCount: uploadSources.filter((source) => source.origin === 'upload').length,
    privateSourceCount: uploadSources.filter((source) => source.origin === 'private').length,
