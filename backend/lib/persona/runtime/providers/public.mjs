...
const PINYIN_SYLLABLES = [
  ...
  'xiao', 'lin', 'ang', 'eng', 'ong', 'iao', 'ian', 'iang', 'uan', 'uang', 'uai', 'uei',
  'jie', 'jun', 'liu', 'tang', 'zhu', 'ui',
  ...
  try {
    ...
    return {
      ...
      metadata: {
        ...
        outboundScholarUrls: extractScholarProfileLinks(html)
      }
    };
  } catch {
    return null;
...
function homepageHandleCandidates(mentor) {
  ...
  return uniqueBy([
    ...
    given && last ? `${pinyinGivenInitials}${last}` : '',
...
function personalDomainCandidateUrls(mentor) {
  const handles = homepageHandleCandidates(mentor).filter((handle) => handle.length >= 3 && handle.length <= 24);
  const urls = [];
...
function institutionHomepageCandidateUrls(mentor) {
  ...
  if (domains.includes('tsinghua.edu.cn')) {
    for (const handle of handles) {
      urls.push(
        `https://pi.cs.tsinghua.edu.cn/person/${handle}/`,
        `https://pi.cs.tsinghua.edu.cn/lab/people/${handle}/en/`,
        `https://pi.cs.tsinghua.edu.cn/lab/people/${handle}/`,
    ...
    urls.push('https://pi.cs.tsinghua.edu.cn/people/#faculty');
  }
...
function sourceLooksLikeMentorHomepage(source, mentor) {
  ...
  const isInstitutionDomain = institutionDomainHints(mentor.affiliation).some((domain) => url.includes(domain));
  if (!isInstitutionDomain && earlyContent.length < 500) return false;
...
async function discoverInstitutionHomepages(mentor, config) {
  const urls = uniqueBy([
    ...institutionHomepageCandidateUrls(mentor),
    ...personalDomainCandidateUrls(mentor)
  ], (item) => item);
...
function buildSearchQueries(mentor) {
  ...
  if (domainHints.includes('tsinghua.edu.cn')) {
    queries.unshift(
      `site:pi.cs.tsinghua.edu.cn "${name}"`,
...
export async function collectPublicSources({ mentor, publicUrls = [], config, skipPublicSearch = false, disableOpenAlex = false }) {
  ...
  if (!skipPublicSearch) {
    ...
    for (const page of await discoverInstitutionHomepages(mentor, config)) {
      sources.push(page);
      for (const scholarUrl of page.metadata?.outboundScholarUrls || []) {
        sources.push(scholarProfileHintSource({
          url: scholarUrl,
          title: `Google Scholar profile for ${mentor.name}`,
          provider: 'page-link-discovery',
          query: page.metadata?.query,
          snippet: `Discovered from fetched page: ${page.title}`,
